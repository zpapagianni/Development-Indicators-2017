...
#Arable.Land.pcnt
mybins_elec<-seq(0,50,10)
mypalette_elec <- colorBin(palette="Purple", domain=join.coord$Access2Elec.pcnt, na.color="transparent", 
                                    bins=mybins_elec)
##Arable.Land
leaflet(my_spdf) %>% addTiles()  %>% 
  setView(lat=10, lng=0 , zoom=2) %>%
  addPolygons( 
    fillColor = ~mypalette_elec(join.coord$Access2Elec.pcnt), 
    stroke=TRUE, 
    fillOpacity = 0.9, 
    color="white", 
    weight=0.3) %>% 
  addLegend( 
    pal=mypalette_elec, 
    values=~join.coord$Access2Elec.pcnt, 
    opacity=0.9, title = "Access to Electricity (%)", 
    position = "bottomleft" )
```
The plot below shows only countries with Access to Electricity less than 100%.We can see that all countries are categorised as either lower or upper middle income. 
```{r}
dev.inc %>% 
...
leaflet(my_spdf) %>% addTiles()  %>% 
  ...
  addLegend( 
    ...
    opacity=0.9, title = "Education Exp.(%)", 
    position = "bottomleft" )
```
